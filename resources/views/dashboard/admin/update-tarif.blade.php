<x-app-layout>
  ...
  <section class="flex items-center w-full gap-5 p-5 bg-white rounded-lg shadow-primary">
    ...
    <form action="{{ route('admin.update-tarif', $tarif->id) }}" method="POST"
      ...
      <div class="space-y-3 w-[50%]">
        <div class="">
          ...
          <x-ui.content.form.input-field name="jenis_plg" value="{{$tarif->jenis_plg}}" type="text" id="jenis_plg"
            required />
        </div>
        <div class="">
          <x-ui.content.form.lable title="Biaya Beban" />
          <x-ui.content.form.input-field name="biaya_beban" value="{{$tarif->biaya_beban}}" type="number"
            id="biaya_beban" required />
        </div>
        <div class="">
          <x-ui.content.form.lable title="Tarif per KWH" />
          <x-ui.content.form.input-field name="tarif_kwh" value="{{$tarif->tarif_kwh}}" type="number" id="tarif_kwh"
            required />
        </div>
